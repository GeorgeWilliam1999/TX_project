...
**Owner:** G. Scriven · **Started:** 2026-06-10 · **Status:** F0–F3 complete (2026-06-11);
F4a+F4b done (2026-06-14): extrapUTT reproduced @ 14.9 µm; **on the REAL v8r1 field the chart
loses to extrapUTT ~260× (4.7 mm vs 14.9 µm)** — the 12.1 µm headline is toy-field-only. ADR 0011 ruling + chart redesign next.
...
floor of the first-order analytic-kick approach **on the toy field**. ⚠️ **F4b CAVEAT (06-14):
these 12.1 µm are on the artificially-smooth toy `twodip` field. On the REAL LHCb v8r1 field
the same chart only reaches ~4.7 mm median — the global even-multipole basis fits real-field
B_y to just ~8% — and loses to the incumbent extrapUTT (14.9 µm) by ~260×. The flattening
*principle* holds on smooth fields; the *global-multipole representation* does not survive the
real field.** **Next: ADR 0011 ruling + a localised-representation chart redesign.**
...
- **F4a ✅ DONE 06-14:** faithful Python `extrapUTT` (`paper_p0/extraputt_py.py`,
  bit-reproduces the C++ CSVs). **Sane incumbent = 14.9 µm median / 2178 µm p95** on
  PV-pointing tracks vs the **real v8r1 field**, m_polarity=−1, qOp=corpus_qop/299.792458.
  The 369 mm "broken baseline" was NOT a wiring bug — it was (1) field mismatch (corpus
  RK truth is the toy `twodip` field; extrapUTT's 25v0 coeffs are fit to v8r1 — they differ
  by 473 mm median on UT→T), (2) population mismatch (extrapUTT is a local expansion around
  the PV-pointing manifold; the uniform corpus is 94 % out-of-grid), (3) a polarity-sign red
  herring. See `results/F4a_extraputt_validated_2026-06-14.md`.
- **F4b ✅ DONE 06-14 — the chart LOSES to extrapUTT on the real field.** Rebuilt the chart
  on v8r1 (`charts/build_chart_v8r1.py` → `chart_tables_v8r1.npz`, self-consistent κ0=1.017e-3)
  and scored on the v8r1 PV-pointing pool: **chart 4725 µm median (best-tuned 3868) vs
  extrapUTT 14.9 µm vs straight 237 mm** — extrapUTT wins ~260×. Root cause: the global O12
  even-multipole basis reconstructs the real v8r1 B_y only to ~8% (vs ~2% on the smooth toy
  field; max residual 1.73 T = Runge ringing on localised fringe structure), and higher order
  doesn't help. extrapUTT wins because it is a per-bin (60×50) local degree-9 fit. **The 12.1 µm
  headline was a toy-field artifact; it does not transfer.** See
  `results/F4b_bakeoff_v8r1_2026-06-14.md`. Consequence: F5 deployment of the *current* chart
  is not justified; competing needs a LOCALISED field representation (chart redesign, new phase).
